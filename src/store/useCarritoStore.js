 ...
 * - nombre: string
 * - precioBase: number
 * - extrasSeleccionados: Array<{ id: number, nombre: string, precioExtra: number, cantidad?: number }>
 * - observaciones: string
 * - cantidad: number
 * - precioUnitario: number = precioBase + sum(extrasSeleccionados.precioExtra * cantidad)
...
const getExtrasPriceSum = (item) => {
  ...
  return ex.reduce((acc, e) => {
    ...
    return acc + Number(p) * getExtraCantidad(e);
  }, 0);
};
    ...
    (set) => ({
      ...
      addItem: (item) => {
        ...
        set((state) => {
          ...
          const incomingFingerprint = getExtrasFingerprint({
            extrasSeleccionados: incomingExtras,
          });

          const existingIndex = state.items.findIndex((currentItem) => {
            const currentFingerprint = getExtrasFingerprint(currentItem);
            return (
              currentItem.slug === item.slug &&
              (currentItem.articuloId ?? currentItem.slug) ===
                (item.articuloId ?? item.slug) &&
          ...
          const extrasSum = incomingExtras.reduce(
            (acc, e) =>
              acc + Number(e.precioExtra ?? e.precio ?? 0) * getExtraCantidad(e),
            0
          );
          ...
          const newItem = {
            ...
            articuloId:
              item.articuloId ?? (item.slug ? Number(item.slug) || item.slug : null),
